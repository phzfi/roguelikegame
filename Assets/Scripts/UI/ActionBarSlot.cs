...
public class ActionBarSlot : MonoBehaviour, IDropHandler
{
    ...
    private GameObject MakeUsableButton(GameObject original)
    {
        ...
        var potion = original.GetComponent<HealthPack>();
        if(potion != null)
            button.onClick.AddListener(() => { potion.UseHealthPack(usableButton); }); //to test with potions
        ...
        usableButton.AddComponent<HealthPackColorChanger>();
